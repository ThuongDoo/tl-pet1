        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-2">
          <div className="flex items-center gap-4">
            ...
            <a
              ...
              className="hidden items-center gap-1.5 hover:text-primary sm:flex"
            >
              <svg
                className="h-3.5 w-3.5"
                viewBox="0 0 24 24"
                fill="currentColor"
              >
                <path d="M2 6a2 2 0 012-2h16a2 2 0 012 2v12a2 2 0 01-2 2H4a2 2 0 01-2-2V6zm2 0l8 6 8-6" />
              </svg>
              {data.brand.email}
            </a>
          </div>
                  ...
                  <ul className="invisible absolute left-0 top-full min-w-64 rounded-xl border border-slate-100 bg-white py-2 opacity-0 shadow-xl transition group-hover:visible group-hover:opacity-100">
                    {link.children.map((child) => (
                      <li key={child.label}>
                        <a
                          href={child.href}
                          className="block px-4 py-2 text-sm text-slate-600 hover:bg-primary/10 hover:text-primary"
                        >
                          {child.label}
                        </a>
                      </li>
          ...
          <ul className="flex flex-col gap-1">
            {NAV_LINKS.map((link) => (
              <li key={link.label}>
                <a
                  href={link.href}
                  className="block rounded-lg px-3 py-2 text-sm font-semibold text-ink hover:bg-primary/10"
                  ...
                  <ul className="ml-3 flex flex-col gap-1 border-l border-slate-100 pl-3">
                    {link.children.map((child) => (
                      <li key={child.label}>
                        <a
                          href={child.href}
                          className="block rounded-lg px-3 py-1.5 text-sm text-slate-500 hover:bg-primary/10 hover:text-primary"
                          onClick={() => setOpen(false)}
                        >
                          {child.label}
                        </a>
